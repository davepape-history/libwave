...
void add_texverts_area(wfObject *obj,struct _params *param);
void compute_normal(wfFace *face,wfObject *obj,float *norm);
void normalize(float *v);
void cross_product(float *a,float *b,float *prod);
...
main(int argc,char **argv)
{
 ...
 else if (!strcasecmp(argv[optind+2],"area"))
	add_texverts_area(obj,&param);
 else
	{
...
void add_texverts_area(wfObject *obj,struct _params *param)
{
 ...
 wfVertex min,max;
 int i,ntexc;
 float norm[3],anorm[3],uaxis[3],vaxis[3],dot;
 ...
 for (p=obj->parts, ntexc=0; p; p=p->next)
	...
	anorm[1] = fabs(norm[1]);
	anorm[2] = fabs(norm[2]);
	if ((anorm[0] >= anorm[1]) && (anorm[0] >= anorm[2]))
		{
		uaxis[0] = 0; uaxis[1] = 0; uaxis[2] = -1;
		}
	else
		{
		uaxis[0] = 1; uaxis[1] = 0; uaxis[2] = 0;
		}
	dot = uaxis[0]*norm[0] + uaxis[1]*norm[1] + uaxis[2]*norm[2];
	uaxis[0] -= dot*norm[0];
	uaxis[1] -= dot*norm[1];
	uaxis[2] -= dot*norm[2];
	normalize(uaxis);
	cross_product(norm,uaxis,vaxis);
	for (i=0; i<face->nverts; i++, ntexc++)
		{
		wfVertex *v = &(obj->vert[face->vert[i]-1]);
		face->tex[i] = ntexc+1;
		obj->texc[ntexc][0] = ((*v)[0]-min[0])*uaxis[0] +
			((*v)[1]-min[1])*uaxis[1] +  ((*v)[2]-min[2])*uaxis[2];
		obj->texc[ntexc][1] = ((*v)[0]-min[0])*vaxis[0] +
			((*v)[1]-min[1])*vaxis[1] +  ((*v)[2]-min[2])*vaxis[2];
		}
	}
}
...
{
 float len;
 len = fsqrt(v[0]*v[0] + v[1]*v[1] + v[2]*v[2]);
 v[0] /= len;
 v[1] /= len;
 v[2] /= len;
}
